...
@Component({
    selector:      'gd-graph-wrapper1',
    templateUrl: "./gd-graph-wrapper1.component.html",
    styleUrls:  ["./gd-graph-wrapper1.component.scss"],
    encapsulation: ViewEncapsulation.Emulated
})
export class GDGraphWrapper1Component implements AfterViewInit, OnDestroy
{
    ...
    { 
        ...
        for( let i = 0; i < this.tooltipModel.body.length; ++i )
        {
            for( let j = 0; j < this.tooltipModel.body[i].lines.length; ++j )
            {
        ...
        let txtSize = GDCommonTools.MeasureText( tooltip, 'monospace', 'normal', '12px' );
    ...
    public SetTheme( theme: string )
    {
        if( theme === 'light' )
        {
            this.legendColor    = '#888';
            this.axeLabelsColor = '#888';
            this.gridLines      = '#aaa';
            this.gridLinesZero  = '#555';
        }
